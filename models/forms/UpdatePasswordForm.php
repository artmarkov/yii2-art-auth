...
class UpdatePasswordForm extends Model
{
    ...
    {
        return [
            'current_password' => Yii::t('art/auth', 'Current password'),
    ...
    public function validateCurrentPassword()
    {
        if (!Yii::$app->art->checkAttempts()) {
            $this->addError('current_password', Yii::t('art/auth', 'Too many attempts'));
            return false;
        }

        if (!Yii::$app->security->validatePassword($this->current_password, $this->user->password_hash)) {
            $this->addError('current_password', Yii::t('art/auth', "Wrong password"));
        }
    }
